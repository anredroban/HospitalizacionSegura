...
/*
 ...
 var uName="PlusWireless";
 var passwrd="eM6btmVPufgXobhbGG#hcH7v";
 jQuery.ajax({
 type: "POST",
 url: "https://linkapi",
 dataType: 'json',
 ...
 if(response.link){
 jQuery('#btnGuardarAPI').after('<span class="respuesta"><a target="_blank" class="btn btn-primary" href="'+response.link+'">Realizar Pago</a></span>');
 jQuery('#btnGuardarAPI').append('<input type="hidden" name="post_id" value="'+response.id+'" />');
 }else{
 jQuery('#control3').after('<label class="respuesta">'+response.mensaje+'</div></label>');
 }
 },
 ...
 (function foo() {
 var post_id = jQuery('#idform').find('input[name="post_id"]').val();
 jQuery.ajax({
 type: "POST",
 url: "https://plusmedical.com.ec/wp-json/api/v1/check_masivos",
 dataType: 'text',
 ...
 jQuery('#control3').after('<label class="pago">'+response+'</label>');
 }else{
 jQuery('.pago').text(response);
 }
 }else{
 setTimeout(foo, 2500);
 }
...
function validateManagementData(){
    ...
    if($productoPrincipalDiv.is(":visible")){
       ...
       /* if($("#provinciaAsegurada").val() == ""){
        ...
        if($("#generoGestion").val() == ""){
            alert("Campo genero vacio");
            $isValid = false;
            return;
        }
